...
import com.changgou.order.pojo.Payment;
import com.changgou.order.service.PaymentService;
...
public class PaymentController {
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "Payment对象",value = "传入JSON数据",required = false) Payment payment,@PathVariable  id){
        //设置主键值
        payment.set(id);
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "")
    @GetMapping("/{id}")
    public Result<Payment> findById(@PathVariable  id){
